    ...
    <div class="row">
        <div class="col-md-12">
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" placeholder="{{ trans('news.title') }}" class="form-control {{ !empty(get_error('title'))?'is-invalid':'' }} " value="{{ old('title') }}"/>
            </div>
        </div>
        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                <input type="content" id="content" name="content" placeholder="{{trans('news.content')}}" class="form-control {{ !empty(get_error('content'))?'is-invalid':'' }} " value="{{ old('content') }}"/>
            </div>
        </div>
        <div class="col-md-12">
            <div class="form-group">
                <label for="description">{{ trans('news.description') }}</label>
                <textarea name="description" id="description" placeholder="{{trans('news.description')}}" class="form-control {{ !empty(get_error('description'))?'is-invalid':'' }} ">{{ old('description') }}</textarea>
            </div>
        </div>
    </div>
    <input type="submit" class="btn btn-success" value="{{ trans('news.create') }}">
</form>
<?php
